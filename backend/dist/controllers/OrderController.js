...
const get_order = (req, res) => {
    ...
    if (req.params.id == 'undefined')
        order_1.default.find().sort({ date_added: -1 }).then(orders => { res.status(200).json(orders); }).catch(err => {
            console.error(err);
            res.status(500).json({ error: 'An error occurred while fetching orders' });
        });
    else
        order_1.default.find().sort({ date_added: -1 }).then(orders => { res.status(200).json(orders.filter(item => String(item.user) === req.params.id)); }).catch(err => {
            console.error(err);
            res.status(500).json({ error: 'An error occurred while fetching orders' });
        });
};
...
const update_order = (req, res) => {
    order_1.default.findByIdAndUpdate({ _id: req.params.id }, { $set: { stepNo: req.body.stepNo } }).then(order => {
        order_1.default.find().sort({ date_added: -1 }).then(orders => res.json(orders));
    });
};
