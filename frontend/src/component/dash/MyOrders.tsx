...
interface Column {
    id: 'id' | 'product' | 'date' | 'mode' | 'cost' | 'status' | 'action' | 'customer' | 'phone';
    label: string;
    minWidth?: number;
    align?: 'left';
...
export default function MyOrders() {
  ...
  const getCustomerFromId = (id: number) => {
    if(user.user?.state == 1){
      const filteredCustomer = customer.customers.filter(item => item._id?.toString() == id.toString())
    ...
    orders.orders.map(item => {
      ...
      if(product)
        rows.push(
          createData(
            item._id!,
            product.title + (product.caliber ? ' ' + product.caliber + 'mm' : '') + (item.items.length > 1 ? ` and ${item.items.length} more` : ''),
            currCustomer!.name,
            currCustomer!.phone,
  ...
  return (
    <ThemeProvider theme={theme}>
        ...
        <Box sx={{padding:'20px', font:'bold black'}} className='flex sm:flex-row flex-col justify-between border-b-[1px] border-gray'>
        ...
        <Table sx={{ minWidth: 770 }} aria-label="custom pagination table">
            ...
            <TableBody>
                {rows!.filter((row:any) => 
                                row.product.toLowerCase().indexOf(searchStr.toLowerCase()) != -1 
                                || row.customer.toLowerCase().indexOf(searchStr.toLowerCase()) != -1)
                          ...
                          <TableCell key={'Action'} align='left'>
                            <IconButton 
                              ...
                              onClick={() => {
                                ...
                                navigate(`/${user.user!.state == 1 ? 'manage' : 'dashboard'}/track/${row.id}`) 
                              }} 
                              data-id={row.id}>
