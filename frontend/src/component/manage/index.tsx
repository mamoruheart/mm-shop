...
export const Manage = () => {
  ...
  const [customerRows, setCustomerRows] = useState<CustomerData[]>([]);
  ...
  useEffect(() => {
    ...
    dispatch(getOrders(navigate, "undefined"));
  ...
  useEffect(() => {
    ...
    const parsedData: Data[] = products.products.map((item: any) => ({
      ...
      detail: `${
        item.caliber != 0
          ? `${item.caliber}MM x ${item.barrelLength}MM ${item.actionType}`
          : item.description
      }`
    }));
        ...
        <Grid container spacing={3} className="max-w-screen-xl px-8">
          <Grid md={3} xs={12} className="border border-white md:h-screen p-2">
            <ul className="mt-5">
              <div>
                <CardContent className="bg-white border border-[#eff2ff] rounded-lg flex flex-row gap-x-3">
                  ...
                  >
                    ...
                    />
                    <div className="flex flex-col justify-center pl-1 text-lg">
                      Welcome Eric
                    </div>
                  </Typography>
              ...
              <MenuList
                sx={{ marginTop: "30px" }}
                className="bg-white border border-[#eff2ff] rounded-lg"
              >
