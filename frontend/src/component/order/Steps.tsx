...
export default function OrderStepper(props) {
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Box sx={{ maxWidth: 400}} >
        <Stepper activeStep={stepNo!} orientation={window.innerWidth >= 768 ? 'vertical' : 'horizontal'} className='mt-2.5 overflow-x-auto'>
          {steps.map((step, index) => (
            <Step key={step.label}>
              ...
              <StepContent>
                <Typography>{window.innerWidth >= 768 ? (step.description) : ''}</Typography>
                {
