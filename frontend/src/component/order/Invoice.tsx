import { faEnvelope, faLocation, faPhone, faUser } from "@fortawesome/free-solid-svg-icons";
    ...
    useEffect(() => {
      if(customer && currentOrder){
        if(user!.state == 1){
          ...
          const filteredCustomer = customer.customers.filter(item => item._id == currentOrder!.user!.toString())
    ...
    return (
        <>
            ...
            <div className="sm:col-span-6">
                ...
                <label className="block text-md font-medium leading-6 text-gray-900">
                    Invoice Date: {currentOrder ? currentOrder.date_added?.toString() : ''}
                </label>
                <label className="block text-md font-medium leading-6 text-gray-900">
                    FFL: <a className="text-blue-500 underline-offset-2" href={UPLOAD_URI + currentOrder?.ffl}>Donwload FFL</a>
                </label>
                {/* <label className="block text-md font-medium leading-6 text-gray-900">
            ...
            <div className="sm:col-span-3">
              ...
              <ul>
                ...
                <li className="block text-md font-medium leading-6 text-gray-900">
                    <FontAwesomeIcon icon={faLocation} className="pr-3"/> {CONTACT.address}
                </li>
                <li className="block text-md font-medium leading-6 text-gray-900">
                    <FontAwesomeIcon icon={faPhone} className="pr-3"/> {CONTACT.phone}
                </li>
                <li className="block text-md font-medium leading-6 text-gray-900">
                    <FontAwesomeIcon icon={faEnvelope} className="pr-3"/> {CONTACT.email}
                </li>
              </ul>
              <div className="mt-2">
              ...
              <label className="block text-md font-medium leading-6 text-gray-900 pb-2">
              ...
              </label>
                {
                  user!.state == 1 ? (
                    <ul>
                      <li className="block text-md font-medium leading-6 text-gray-900">
                      <FontAwesomeIcon icon={faUser} className="pr-3"/> {currentCustomer ? currentCustomer!.name : ''}
                      </li>
                      <li className="block text-md font-medium leading-6 text-gray-900">
                          <FontAwesomeIcon icon={faLocation} className="pr-3"/> {currentCustomer ?  currentOrder?.billAddress?.city + ' ' + currentOrder?.billAddress?.state + ' ' + currentOrder?.billAddress?.zip : ''}
                      </li>
                      <li className="block text-md font-medium leading-6 text-gray-900">
                          <FontAwesomeIcon icon={faPhone} className="pr-3"/> {currentCustomer ?  currentCustomer!.phone : ''}
                      </li>
                      <li className="block text-md font-medium leading-6 text-gray-900">
                          <FontAwesomeIcon icon={faEnvelope} className="pr-3"/> {currentCustomer ?  currentCustomer!.email : ''}
                      </li>
                    </ul>
                  ) : (
                    <ul>
                      ...
                      <li className="block text-md font-medium leading-6 text-gray-900">
                        <FontAwesomeIcon icon={faLocation} className="pr-3"/> {currentOrder?.billAddress?.city + ' ' + currentOrder?.billAddress?.state + ' ' + currentOrder?.billAddress?.zip}
                      </li>
                    </ul>
                  )
                }
                   ...
                   currentOrder?.items.map(item => {
                    ...
                    return(
                      <ul className="sm:px-10 sm:flex sm:w-full sm:justify-between">
                        ...
                        <div className="flex flex-col items-start justify-between">
                          <h2 className="text-md text-gray-900"> {item.amount} Piece(s)</h2>
                        </div>
                        <div className="flex items-center space-x-4">
                          <p className=" font-bold text-md">$ {item.amount! * product.price} (Total)</p>
                        </div>
                      </ul>
                    );
                   })
            ...
            <div className="sm:col-span-6">
              ...
              <div className="mt-2">
                ...
                <ul className="sm:px-10 sm:flex sm:w-full sm:justify-between">
                  ...
                  <div className="flex items-center space-x-4">
                    <p className=" font-bold text-md">+ ${(totalPrice * FEE).toFixed(2)}</p>
                  </div>
                </ul>
                <ul className="sm:px-10 sm:flex sm:w-full sm:justify-between">
                  <div className="flex flex-col items-start justify-between">
