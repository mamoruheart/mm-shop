...
const initialState: OrderState = localStorageData
  ? localStorageData.order
  : {
      orders: [],
      loading: false,
      error: null
...
export const getOrders =
  (navigate: any, id: string) => async (dispatch: AppDispatch) => {
    ...
    createAxiosInstance(navigate)
      ...
      .catch((error: AxiosError) => {
        if (error.response && error.response.data) {
          ...
          dispatch(fetchOrdersFailure(errorData));
        } else {
...
export const deleteOrder =
  (navigate: any, deleteOrderId: string) => async (dispatch: AppDispatch) => {
    dispatch(fetchOrderStart());
